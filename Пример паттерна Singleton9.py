'''
Подвиг 10 (на повторение материала). В программе предполагается реализовать 
парсер (обработчик) строки (string) в определенный выходной формат. Для этого
...
class Factory:
    ...
    def build_number(self, string):
        # s = lambda (map(float,string.split())) 
        # x = ' '.join(map(str,s))
        return float(string)
